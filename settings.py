...
def settings_tab():
    with gr.Blocks() as demo:
        ...
        with gr.Tab("Settings"):
            ...
            # Add checkboxes for Default Agents and Custom Agents
            ...
            with gr.Blocks():
                ...
                for agent, role in custom_agent_roles.items():
                    with gr.Row() as row:
                        ...
                        remove_button.click(
                            fn=lambda agent=agent, role=role, custom_agent_roles=custom_agent_roles, unsaved_changes_indicator=unsaved_changes_indicator: remove_agent_role(agent, role, custom_agent_roles, unsaved_changes_indicator),
                            inputs=[],
                            outputs=[custom_agent_roles_display, unsaved_changes_indicator]
                        )
                    agent_role_rows.append(row)
...
def remove_agent_role(agent, role, custom_agent_roles, unsaved_changes_indicator):
    if agent in custom_agent_roles:
        del custom_agent_roles[agent]
    return custom_agent_roles, gr.Markdown("**Unsaved Changes**", visible=True)

# Function to add a new agent role
def add_agent_role(agent, role, custom_agent_roles_display, unsaved_changes_indicator, new_agent_textbox, new_role_textbox):
    custom_agent_roles = custom_agent_roles_display
    if agent and role:
        custom_agent_roles[agent] = role
        return custom_agent_roles, "", "", gr.Markdown("**Unsaved Changes**", visible=True)
    return custom_agent_roles, new_agent_textbox, new_role_textbox, unsaved_changes_indicator
...
def save_custom_agent_roles(custom_agent_roles_display, unsaved_changes_indicator):
    ...
    return status_message, gr.Markdown("**Unsaved Changes**", visible=False)
